  ...
  <script type="module">
    ...
    window.lb = {{
      update: async (score, player) => {{
        ...
        if (typeof player !== 'string' || !/^[a-zA-Z0-9_]+$/.test(player) || player.length < 1 || player.length > 20) return;
        try {{
      ...
      listen: (cb) => {{
        ...
        return onSnapshot(q, snap => cb(snap.docs.map(d => d.data())), err => console.error(err));
      }}
    }};
  </script>

  <style>
    :root {{ --bg: #000814; --neon: #00ff88; --glow: #00ff80; --yellow: #ffff40; --purple: #ff00ff;
             --glass: rgba(10,25,15,0.18); --font: 'Orbitron','Courier New',monospace; }}
    * {{ margin:0; padding:0; box-sizing:border-box; }}
    ...
    .glass {{ background:var(--glass); backdrop-filter:blur(14px); -webkit-backdrop-filter:blur(14px);
              ...
              box-shadow:0 0 12px rgba(0,255,136,0.5); transition:all .3s; pointer-events:auto; }}
    ...
    #score {{ top:16px; left:50%; transform:translateX(-50%); font-weight:700; color:var(--yellow);
              font-size:1.5em; text-shadow:0 0 16px var(--yellow); }}
    #vr-status {{ top:16px; right:16px; background:rgba(0,255,128,0.2); color:#000; padding:8px 14px;
    ...
    #chat-input {{ width:68%; background:rgba(0,0,0,0.4); color:var(--neon); border:1px solid var(--purple);
                   padding:8px; border-radius:6px; font-family:var(--font); backdrop-filter:blur(4px); }}
    #legal {{ position:fixed; bottom:8px; left:50%; transform:translateX(-50%); font-size:.7em; opacity:.7;
...
<body>
  ...
  <div id="chat" class="glass" role="region" aria-label="Chat">
    ...
    <button onclick="sendChat()" aria-label="Enviar">SEND</button>
  </div>
  ...
  <div id="donate-crypto" class="glass">
    ...
    <button onclick="copyWallet()" style="margin:6px auto; display:block; font-size:.9em; padding:8px 16px;">
      0x2bd4...2fea
    </button>
  ...
  <div id="legal">
    <a href="javascript:showPrivacy()">Privacidad</a> • 
    <a href="javascript:showTerms()">Términos</a> • 
  ...
  <script type="module">
    ...
    if (!localStorage.getItem('slyverse_name')) {{
      const name = prompt('Tu alias (1-20 chars, solo letras/números/_):', playerName);
      if (name && /^[a-zA-Z0-9_]+$/.test(name) && name.length <= 20) {{
        playerName = name; localStorage.setItem('slyverse_name', name);
      }}
    }}
    ...
    function spawnFood() {{
      ...
      food = new THREE.Mesh(new THREE.SphereGeometry(0.7,16,16),
        new THREE.MeshBasicMaterial({{color:0xffff40, emissive:0xffff40, emissiveIntensity:0.6}}));
      food.position.set(Math.random()*40-20, 0.5, Math.random()*40-20); scene.add(food);
    }} spawnFood();
    ...
    function beep(f=440,d=100){{const a=new AudioContext(),o=a.createOscillator(),g=a.createGain();
      o.frequency.value=f; g.gain.setValueAtTime(0.3,a.currentTime);
    ...
    let breath=0;
    function animate() {{
      breath += 0.012; const s = 1 + Math.sin(breath)*0.08;
      grid.scale.set(s,1,s); light.intensity = 1.8 + Math.sin(breath*3)*0.6;
      ...
      if (!renderer.xr.isPresenting) {{
        ...
        if (session && session.inputSources) for (const src of session.inputSources)
          if (src.gamepad && src.gamepad.axes) {{
            const [x,y]=src.gamepad.axes;
            if (Math.abs(x)>0.7) dir={{x:Math.sign(x),z:0}};
            if (Math.abs(y)>0.7) dir={{x:0,z:Math.sign(y)}};
          }}
      }}
      ...
      if (food && Math.abs(nx-food.position.x)<1 && Math.abs(nz-food.position.z)<1) {{
        score += 10; scoreEl.innerText = `COILS: ${{score}}`; beep(880,160); spawnFood(); checkLore();
        if (window.lb) window.lb.update(score, playerName);
        if (score >= 300 && score < 310) exportSnakeSVG();
      }} else scene.remove(snake.pop());

      renderer.render(scene, camera);
    }} renderer.setAnimationLoop(animate);

    function reset() {{
      snake.forEach(s=>scene.remove(s)); snake.length=0;
      head = new THREE.Mesh(headGeo, headMat); head.position.set(0,0.5,0); scene.add(head); snake.push(head);
      score=0; scoreEl.innerText='COILS: 0'; spawnFood();
    }}
    ...
    function checkLore(){{ lore.forEach((l,i)=>{{ if(score>=l.score && i>=unlockedLore){{
      unlockedLore++; showLore(l); document.getElementById('lore-progress').innerText=`LORE: ${{unlockedLore}}/7`;
    }}}}); }}
    function showLore(l){{ alert(`LORE DESBLOQUEADO: ${{l.title}}\\n\\n${{l.text}}`); }}

    function exportSnakeSVG() {{
      let svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="-30 -30 60 60" style="background:#000">`;
      snake.forEach((s,i)=>{{ const [x,z]=[s.position.x,s.position.z]; const h=120+(i*5)%240;
        svg+=`<rect x="${{x-0.5}}" y="${{z-0.5}}" width="1" height="1" fill="hsl(${{h}},100%,50%)"/>`;}});
      svg+=`</svg>`;
      const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([svg],{{type:'image/svg+xml'}}));
      a.download=`slyverse_${{playerName}}_${{score}}.svg`; a.click();
    }}

    function sendChat(){{ const i=document.getElementById('chat-input'), m=i.value.trim();
      if(m && m.length<=100){{ const c=document.getElementById('chat-list');
        c.innerHTML+=`<div style="margin:4px 0;opacity:.9"><strong>${{playerName}}:</strong> ${{m}}</div>`;
        c.scrollTop=c.scrollHeight; i.value='';
      }}
    }}

    if(window.lb) window.lb.listen(s=>document.getElementById('lb-list').innerHTML=
      s.map(p=>`<div style="margin:3px 0">${{p.player}}: <strong>${{p.score}}</strong></div>`).join('') ||
    ...
    function copyWallet(){{ navigator.clipboard.writeText('{WALLET_ADDRESS}').then(()=>{{
      const b=event.target, old=b.innerText; b.innerText='¡Copiado!'; b.style.background='var(--yellow)'; b.style.color='#000';
      setTimeout(()=>{{
        b.innerText=old; b.style.background=''; b.style.color='';
      }},1500);
      const q=document.getElementById('donate-qr');
      if(!q.innerHTML){{ q.innerHTML=`<img src="https://api.qrserver.com/v1/create-qr-code/?size=120x120&data={WALLET_ADDRESS}" alt="QR" style="border-radius:8px;box-shadow:0 0 12px var(--neon)">`;
        q.style.opacity='1'; q.style.transition='opacity .5s';
      }}
    }}); }}
